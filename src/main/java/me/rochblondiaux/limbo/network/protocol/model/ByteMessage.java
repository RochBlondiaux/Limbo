...
public class ByteMessage extends ByteBuf {
    ...
    public void writeNamelessCompoundTag(BinaryTag binaryTag) {
        try (ByteBufOutputStream stream = new ByteBufOutputStream(buf)) {
            ...
            } else if (binaryTag instanceof ByteBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof ShortBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof IntBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof LongBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof DoubleBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof StringBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof ListBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof EndBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof FloatBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof LongArrayBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof IntArrayBinaryTag tag) {
                tag.type().write(tag, stream);
            } else if (binaryTag instanceof ByteArrayBinaryTag tag) {
                tag.type().write(tag, stream);
            }
        } catch (IOException e) {
            throw new EncoderException("Cannot write NBT CompoundTag");
    ...
    public void writeNbtMessage(NbtMessage nbtMessage, Version version) {
        if (version.moreOrEqual(Version.V1_20_3)) {
            writeNamelessCompoundTag(nbtMessage.tag());
        } else {
            writeString(nbtMessage.json());
        }
    }
